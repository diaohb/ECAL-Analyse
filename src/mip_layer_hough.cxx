...
void TreeReader::mip_layer(string filename,string pedestal,string outname){
    ...
    (*df).Foreach([&](vector<int> &CellID,vector<int> &HitTag,vector<double> HG_Charge,vector<double> distance_x,vector<double> distance_y){
        ...
        for (int i = 0; i < CellID.size(); i++) {
            if (HitTag.at(i) == 1) {
                ...
                if(layer%2==0){
                    if(abs(distance_x.at(i))<60&&abs(distance_y.at(i))<10){
                        h_mip[CellID.at(i)]->Fill(HG_Charge.at(i));//-m_pedestal[CellID.at(i)]);
                    }
                }
                else{
                    if(abs(distance_x.at(i))<10&&abs(distance_y.at(i))<60){
                        h_mip[CellID.at(i)]->Fill(HG_Charge.at(i));//-m_pedestal[CellID.at(i)]);
                    }
                }
            }
        ...
        //     }
        // }
    },{"CellID","HitTag","HG_Charge","distance_x","distance_y"});
